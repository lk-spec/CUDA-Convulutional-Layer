...
void conv_forward_cpu(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
  ...
  for(int b1=0; b1<B; b1++)                   // for each image in the batch 
  {    for(int m1=0; m1<M; m1++)           // for each output feature maps
  {        for(int h=0; h<H_out; h++)        // for each output element
  {            for(int w=0; w<W_out; w++) 
              {
                  out_4d(b1,m1,h,w) = 0;
                  for(int c1=0; c1<C; c1++)   // sum over all input feature maps
                      {for(int p=0; p<K; p++) // KxK filter
                          {for(int q=0; q<K; q++)
{                              out_4d(b1,m1,h,w) += in_4d(b1,c1,h * S + p,w * S + q) * mask_4d(m1,c1,p,q);
}              }}}}}}
